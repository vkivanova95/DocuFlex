...
class AccessControlTests(TestCase):
    def setUp(self):
        self.business_user = CustomUser.objects.create_user(
            username="business", password="pass", email="business@example.com"
        )
        business_group, _ = Group.objects.get_or_create(name="бизнес")
        self.business_user.groups.add(business_group)

        self.head_user = CustomUser.objects.create_user(
            username="head", password="pass", email="head@example.com"
        )
