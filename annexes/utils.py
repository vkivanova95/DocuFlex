...
def add_annex_header(doc, data, request_obj):
    ...
    subheading = doc.add_paragraph(
        f"към Договор за кредит № {request_obj.loan_agreement.contract_number}"
    )
    subheading.runs[0].bold = True
    subheading.alignment = WD_PARAGRAPH_ALIGNMENT.CENTER

    doc.add_paragraph(
        f'Днес, {data["annex_date"].strftime("%d.%m.%Y")} в гр. {data["city"]}, между:'
    )
...
def add_parties_info(doc, client):
    ...
    p.add_run(
        ", вписано в ТРРЮЛНЦ, ЕИК 111222333, със седалище и адрес на управление: гр. София, в качеството "
    ...
    p.add_run(
        f", вписано в ТРРЮЛНЦ, ЕИК {client.eik}, със седалище и адрес на управление: {address}, представлявано от {representatives}"
    )
...
def add_intro_paragraph(doc, data, request_obj):
    ...
    doc.add_paragraph(
        f'на основание постигнато споразумение се сключи настоящия Анекс № {data["annex_number"]}, '
...
def add_change_sections(doc, data, request_obj, extra_conditions):
    ...
    if data["new_amount_reduction"]:
        section_I.append(
            f"{counter_I}. Ограничава се ползването на кредита, като максималния размер става "
            f"{data['new_amount_reduction']}."
        )
        counter_I += 1
    if data["amount_increase"] and data["new_amount_increase"]:
        section_I.append(
            f"{counter_I}. Увеличава се размера на кредита с {data['amount_increase']}  "
    ...
    if data["repayment_plan"]:
        section_III.append(
            f"{counter_III}. Кредитът се издължава при спазване на погасителен план за намаляване на "
            f"главницата по кредита, съгласно Приложение, неразделна част от настоящия Анекс."
        )
        counter_III += 1
    if data["new_ceiling"]:
        section_III.append(
            f"{counter_III}. Кредитополучателят се задължава да спазва следната схема за ограничения "
    ...
    if data["new_interest"]:
        section_IV.append(
            f"{counter_IV}. Редовната лихва по кредита става {data['new_interest']}"
        )
        counter_IV += 1
    if data["fee_review"]:
        section_IV.append(
            f"{counter_IV}. На датата на подписване на този Анекс Кредитополучателят дължи такса "
            f"разглеждане в размер на {data['fee_review']}."
        )
        counter_IV += 1
    if data["fee_management"]:
        section_IV.append(
            f"{counter_IV}. Такса за управление и обработка (обслужване) на кредита става"
            f" {data['fee_management']} годишно, платима в началото на всяко календарно тримесечие, считано от датата на "
    ...
    if data["no_fees"]:
        section_IV.append(
            f"{counter_IV}. Кредитополучателят не дължи такси по кредита."
        )
        counter_IV += 1
...
def generate_annex_deletion(data, request_obj, extra_conditions=None):
    ...
    doc.add_paragraph()
    doc.add_paragraph(
        f"    1. Във връзка с извършено частично предсрочно погасяване по главница в размер на "
