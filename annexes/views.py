from common.constants import GROUP_MAKER, GROUP_BUSINESS, GROUP_MANAGER
...
class GenerateAnnexView(
    LoginRequiredMixin, GroupRequiredMixin, AnnexPermissionMixin, View
):
    ...
    def get(self, request):
        ...
        return render(
            request, "annexes/start_annex_form.html", {"start_form": start_form}
        )
    ...
    def post(self, request):
        ...
        if "step" not in request.POST:
            ...
            if start_form.is_valid():
                req = get_object_or_404(
                    Request, request_number=start_form.cleaned_data["request_number"]
                )

                # забрана за повторно генериране
                if req.preparation_date:
                    messages.error(
                        request,
                        "Вече е генериран анекс по тази заявка. Повторно генериране не е разрешено.",
                    )
                    return render(
                        request,
                        "annexes/start_annex_form.html",
                ...
                if not req.maker:
                    messages.error(
                        request,
                        "Заявката не е разпределена за работа към изпълнител. Не може да се генерира анекс.",
                    )
                    return render(
                        request,
                        "annexes/start_annex_form.html",
                ...
                formset = (
                    AdditionalConditionFormSet() if annex_type == "standard" else None
                )

                return render(
                    request,
                    "annexes/complete_annex_form.html",
                    {
                        "form": form,
                        "formset": formset,
        ...
        if form.is_valid() and (formset is None or formset.is_valid()):
            req = get_object_or_404(
                Request, request_number=form.cleaned_data["request_number"]
            )
            ...
            if annex_type == "deletion":
                ...
                file_path = generate_annex_standard(
                    form.cleaned_data, req, extra_conditions=extra
                )
        ...
        messages.error(
            request, "Формата съдържа грешки. Моля, проверете въведените данни."
        )
        return render(
            request,
            "annexes/complete_annex_form.html",
...
class AnnexArchiveView(
    ...
    def get_queryset(self):
        queryset = (
            ...
            .filter(request__status=RequestStatus.IN_PROGRESS)
        )
    ...
    def apply_filters(self, queryset):
        ...
        if request_number:
            queryset = queryset.filter(
                request__request_number__icontains=request_number
            )
        if eik:
            queryset = queryset.filter(request__client__eik__icontains=eik)
        if contract_number:
            queryset = queryset.filter(
                request__loan_agreement__contract_number__icontains=contract_number
            )
