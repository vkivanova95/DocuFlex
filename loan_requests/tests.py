...
class RequestModelAndViewTests(TestCase):
    def setUp(self):
        ...
        self.business_user = CustomUser.objects.create_user(username='business', password='pass', email='bus@admin.com')
        self.executor_user = CustomUser.objects.create_user(username='maker', password='pass', email='maker@admin.com')
        ...
        self.contract = Contract.objects.create(client=self.client_obj, contract_number="100-123",currency=self.currency,
                                                start_date='2025-07-01', amount=5000, is_active=True)
        self.request = Request.objects.create(client=self.client_obj, loan_agreement=self.contract, maker=self.business_user,
                                              currency=self.currency,amount=10000, status='В процес на работа', document_type='standard')

    def test_request_default_status(self):
        req = Request.objects.create(client=self.client_obj, loan_agreement=self.contract, currency=self.currency,
                                     amount=5000, maker=self.business_user, document_type='standard')
    ...
    def test_submit_new_request(self):
        response = self.client.post(reverse_lazy('requests:request_submit'), {
