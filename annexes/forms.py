...
class BaseAnnexStartForm(forms.Form):
    request_number = forms.CharField(
        label="Номер на заявка", widget=forms.TextInput(attrs={"class": "form-control"})
    )
...
class AnnexStandardForm(AnnexFormBehaviorMixin, StartFieldsMixin, BaseStyledSimpleForm):
    ...
    def clean(self):
        ...
        if new_amount_increase and not amount_increase:
            self.add_error(
                "amount_increase",
                "Ако е попълнено поле Нов размер, трябва да се попълни и поле Сума на увеличението.",
            )

        return cleaned_data

    new_amount_reduction = forms.CharField(
        required=False,
        widget=forms.TextInput(
            attrs={"class": "form-control", "placeholder": "напр. 100 000.00 лева"}
        ),
    )
    amount_increase = forms.CharField(
        required=False,
        widget=forms.TextInput(
            attrs={"class": "form-control", "placeholder": "напр. 100 000.00 лева"}
        ),
    )
    new_amount_increase = forms.CharField(
        required=False,
        widget=forms.TextInput(
            attrs={"class": "form-control", "placeholder": "напр. 100 000.00 лева"}
        ),
    ...
    stop_disbursement = forms.BooleanField(
        label="Прекратяване на усвояването", required=False
    )

    new_disbursement_date = styled_datefield(
        label="Нова дата на усвояване", required=False
    )
    new_repayment_date = styled_datefield(
        label="Нова дата на погасяване", required=False
    )
    ...
    fee_review = forms.CharField(
        required=False,
        widget=forms.TextInput(
            attrs={"class": "form-control", "placeholder": "напр. 1 000 лева"}
        ),
    )
    fee_management = forms.CharField(
        required=False,
        widget=forms.TextInput(
            attrs={"class": "form-control", "placeholder": "напр. 0,5%"}
        ),
    )
    fee_commitment = forms.CharField(
        required=False,
        widget=forms.TextInput(
            attrs={"class": "form-control", "placeholder": "напр. 0,5%"}
        ),
...
class AnnexDeletionForm(AnnexFormBehaviorMixin, StartFieldsMixin, BaseAnnexStartForm):
    repaid_amount = forms.CharField(
        ...
        required=True,
        widget=forms.TextInput(
            attrs={"class": "form-control", "placeholder": "напр. 100 000 лева"}
        ),
    )
    deed_number = forms.CharField(
        ...
        required=True,
        widget=forms.TextInput(
            attrs={"class": "form-control", "placeholder": "напр. 102/31.01.2024"}
        ),
    )
...
AdditionalConditionFormSet = formset_factory(
    AdditionalConditionForm, extra=1, can_order=False, can_delete=True
)
